...
const MESSAGE_TYPES: Record<string, { label: string; cardClassName: string; fullAvatar: boolean }> = {
  memo: { label: 'Memo', cardClassName: 'is-type-memo', fullAvatar: false },
  invite: { label: 'Invitation', cardClassName: 'is-type-invite', fullAvatar: false },
  photo: { label: 'Photo', cardClassName: 'is-type-photo', fullAvatar: false },
  avatar: { label: 'Avatar', cardClassName: 'is-type-avatar', fullAvatar: true },
  system: { label: 'System', cardClassName: 'is-type-system', fullAvatar: false },
};
...
const BOARD_MESSAGE_SEEDS = [
  {
    ...
    y: 22,
    lines: [
      { kind: 'text' as const, text: 'This is a nice sample letter,' },
      { kind: 'text' as const, text: 'with multiple lines.' },
    ],
  },
  {
    id: 'invite-mariokart',
    ...
    y: 14,
    lines: [
      { kind: 'text' as const, text: 'Want to play Mario Kart later?' },
      { kind: 'text' as const, text: 'I finally unlocked Rainbow Road!' },
    ],
  },
  {
    id: 'photo-sunset',
    ...
    y: 43,
    lines: [
      { kind: 'text' as const, text: 'Check out this sunset from our trip.' },
      { kind: 'text' as const, text: 'It came out better than expected.' },
    ],
  },
  {
    id: 'system-update',
    ...
    y: 39,
    lines: [
      { kind: 'text' as const, text: 'A new Onliine update is available.' },
      { kind: 'link' as const, text: 'View release notes', href: 'https://github.com/ktg5/onliine-electron/releases/latest' },
    ],
  },
  {
    id: 'family-note',
    ...
    y: 63,
    lines: [
      { kind: 'text' as const, text: "Don't forget to call grandma this weekend." },
      { kind: 'text' as const, text: 'She misses you.' },
    ],
  },
  {
    id: 'sports-result',
    ...
    y: 63,
    lines: [
      { kind: 'text' as const, text: 'Your friend beat your tennis score.' },
      { kind: 'text' as const, text: 'Play now and take the lead back!' },
    ],
  },
];

const BOARD_MESSAGES: BoardMessage[] = BOARD_MESSAGE_SEEDS.map((message) => ({
  ...message,
  dateKey: toDateKey(addDays(BOARD_ANCHOR_DATE, message.dayOffset ?? 0)),
}));

const MESSAGES_BY_DATE: Record<string, BoardMessage[]> = BOARD_MESSAGES.reduce((acc, message) => {
  ...
  return acc;
}, {} as Record<string, BoardMessage[]>);

export const MESSAGE_DATE_KEYS = new Set(Object.keys(MESSAGES_BY_DATE));

function getMessagesForDayOffset(dayOffset: number) {
...
function AvatarDisplay({ message, className }: { message: BoardMessage; className: string }) {
  ...
  if (message.miiDataUrl) {
    return <MiiAvatar miiDataUrl={message.miiDataUrl} className={avatarClassName} title={title} alt={message.avatarAlt} fallback={fallback} />;
  }
...
function MessageCard({ message, onOpen }: { message: BoardMessage; onOpen: (id: string) => void }) {
  const type = MESSAGE_TYPES[message.type] ?? MESSAGE_TYPES.memo;
...
function MessageMemo({ message, onClose }: { message: BoardMessage; onClose: () => void }) {
  const type = MESSAGE_TYPES[message.type] ?? MESSAGE_TYPES.memo;
...
interface WiiMessageBoardProps {
  visible: boolean;
  onDisplayedDateChange: (date: string | null) => void;
  targetDate: Date | null;
}

export default function WiiMessageBoard({ visible, onDisplayedDateChange, targetDate }: WiiMessageBoardProps) {
  const [currentDayOffset, setCurrentDayOffset] = useState(0);
  const [selectedMessageId, setSelectedMessageId] = useState<string | null>(null);
  const [dateTransition, setDateTransition] = useState<DateTransition | null>(null);
  const transitionTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const lastTargetDateRef = useRef<number | null>(null);
  ...
  const setDate = (toOffset: number, direction: 'next' | 'prev') => {
  ...
  const renderCards = (messages: BoardMessage[], keyPrefix: string) => (
  ...
  const renderSnapshot = (messages: BoardMessage[], keyPrefix: string) => {
  ...
  return (
    <div
      ...
      style={{ '--message-board-page-slide-ms': `${MESSAGE_PAGE_SLIDE_MS}ms` } as React.CSSProperties}
    >
      ...
      <div className="message-board-holder">
        ...
        >
          <button className="wii-arrow-btn wii-arrow-btn-right" type="button" aria-label="Previous message board day" />
        ...
        >
          <button className="wii-arrow-btn" type="button" aria-label="Next message board day" />
          <button className="wii-track-btn-circle wii-track-btn-base wii-track-btn-no-shadow channel-page-circle" type="button" tabIndex={-1}>
