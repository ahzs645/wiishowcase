...
function buildMaskDataUri(viewBox: string, path: string): string {
...
function getMode(matches: boolean): AspectMode {
  return matches
    ? {
        is43: true,
...
export default function useWiiAspectMode(): AspectMode {
  const [mode, setMode] = useState<AspectMode>(() => getMode(window.matchMedia(MQ).matches));

  useEffect(() => {
    const mql = window.matchMedia(MQ);
    const handler = (e: MediaQueryListEvent) => setMode(getMode(e.matches));
